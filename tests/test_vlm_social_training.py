...
from vlm.social.model import SocialVLM, TextGenerativeVLM
from vlm.social.evidence import GraphBatch
from vlm.social.evaluation import PredictionCollector
from vlm.social.prompt import SOCIAL_SPECIAL_TOKENS
...
def test_route_threshold_none_when_routing_disabled():
    from vlm.social.training import _route_threshold
...
def test_once_sampler_visits_every_selected_row_exactly_once():
    dataset = _TinyControlDataset()
    loader = make_epoch_loader(
        dataset,
        _collate,
        sampling_strategy="once",
    ...
    visited = [
        value
        for batch in loader
        for value in batch["graph_logits"].tolist()
    ]
    assert len(visited) == len(dataset)
    assert sorted(visited) == sorted(dataset.graph.tolist())
...
def test_graph_control_epoch_and_checkpoint_roundtrip(tmp_path):
    ...
    restored_criterion = TaskBCELoss()
...
def test_actual_tiny_qwen_peft_keeps_only_lora_and_new_modules_trainable():
    ...
    token_ids = {
        token: index + 10 for index, token in enumerate(SOCIAL_SPECIAL_TOKENS)
    }
    vlm = SocialVLM(
        backbone,
        token_ids,
...
def test_partition_vlm_parameters_text_mode_has_empty_new_group_and_no_overlap():
    ...
    objective = GenerativeObjective(vlm)
    lora, new = partition_vlm_parameters(objective)
    assert len(lora) == 2
    assert new == []
...
def test_generative_builders_are_fixed_to_text_evidence():
    from vlm.social.training import select_generative_builders

    builders = select_generative_builders(
        {"model": {"output": "yesno", "graph_evidence": "gtoken"}}
    )
    assert builders.graph_evidence == "text"
    assert builders.uses_text_collate is True


def test_generative_builders_follow_only_the_vision_reuse_input_flag():
    from vlm.social.training import select_generative_builders

    enabled = select_generative_builders({
        "model": {"graph_evidence": "gtoken"},
        "input": {"reuse_frozen_vision": True},
    })
    assert enabled.reuse_vision is True
