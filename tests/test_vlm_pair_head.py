...
from vlm.pair_model import PairSocialVLMOutput


def _graph_batch(graph_logits=None) -> PairGraphBatch:
...
def test_zero_init_residual_is_exactly_graph_equivalent():
    decoder = PairTaskResidualDecoder(input_dim=8, hidden_dim=16, dropout=0.0)
    graph = _graph_batch()
...
def test_task_id_routes_to_the_matching_decoder():
    decoder = PairTaskResidualDecoder(input_dim=4, hidden_dim=8, dropout=0.0)
...
def test_task_weighted_bce_matches_manual_binary_loss():
    criterion = PairTaskBCELoss({"lah": 2.0, "laeo": 3.0, "sa": 4.0})
...
def test_bce_rejects_invalid_labels_or_tasks(labels, task_ids, match):
    with pytest.raises(ValueError, match=match):
        PairTaskBCELoss()(torch.zeros(3), labels, task_ids)
...
def test_objective_composes_vlm_yesno_head_and_bce_and_checks_task_order():
    ...
    objective = PairSocialObjective(
        vlm,
        PairYesNoResidualHead(yes_token_id=7, no_token_id=9),
        PairTaskBCELoss(),
    )
    labels = torch.tensor([1.0, 0.0, 1.0])
...
def test_yesno_head_reads_lm_head_and_opens_vlm_gradient():
    graph = _graph_batch()
    vlm = _FakeVLM(torch.randn(3, 8))
    head = PairYesNoResidualHead(yes_token_id=7, no_token_id=9)
    objective = PairSocialObjective(vlm, head, PairTaskBCELoss())
    labels = torch.tensor([1.0, 0.0, 1.0])
