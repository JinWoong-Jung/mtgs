...
def _sample(task, i, j, answer="yes"):
    return SocialSample.from_manifest_record({
...
def test_lah_uses_canonical_looker_to_target_and_na_masks():
    ...
    evidence = assemble_graph_evidence(_sample("lah", 1, 3), cache)
...
def test_laeo_fills_src_and_tgt_for_both_people_and_both_directions():
    cache = _fake_cache()
    evidence = assemble_graph_evidence(_sample("laeo", 1, 3), cache)
...
def test_sa_fills_src_null_in_social_edges_and_both_heatmaps():
    cache = _fake_cache()
    evidence = assemble_graph_evidence(_sample("sa", 0, 2), cache)
...
def test_stack_preserves_fixed_shapes_and_task_masks():
    cache = _fake_cache()
    items = [
        assemble_graph_evidence(_sample("lah", 1, 3), cache),
        assemble_graph_evidence(_sample("laeo", 1, 3), cache),
        assemble_graph_evidence(_sample("sa", 0, 2), cache),
    ]
    batch = stack_graph_evidence(items)
