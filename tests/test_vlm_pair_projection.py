...
from vlm.pair_projection import (
    HeatmapSlotProjector,
    PairEvidenceProjector,
    PersonSlotProjector,
)
...
def _replace(batch: PairGraphBatch, **changes) -> PairGraphBatch:
    values = {
        "tasks": batch.tasks,
...
def test_six_slot_projector_masks_all_absent_raw_values_and_is_trainable():
    batch = _graph_batch()
    projector = PairEvidenceProjector(
