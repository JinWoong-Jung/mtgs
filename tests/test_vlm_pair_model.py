...
from vlm.pair_features import PairGraphBatch, PairGraphEvidence
...
from vlm.pair_prompt import PAIR_SPECIAL_TOKENS, task_conditioned_pair_prompt


def _token_ids():
    return {token: index + 10 for index, token in enumerate(PAIR_SPECIAL_TOKENS)}
...
def _graph_batch() -> PairGraphBatch:
...
def test_pair_wrapper_injects_six_slots_and_reads_social_hidden_with_gradients():
    backbone = _StubBackbone()
    wrapper = PairSocialVLM(
        backbone,
        _token_ids(),
        graph_dim=4,
        graph_hidden_dim=32,
...
def _single_lah_input() -> PairVLMInput:
...
def test_actual_tiny_qwen_forward_keeps_vision_deepstack_and_mrope():
    ...
    token_ids = prepare_pair_tokens(processor.tokenizer, model)
    batch = make_pair_collate(processor)([_single_lah_input()])
