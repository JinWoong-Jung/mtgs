...
from vlm.social.data import SocialAnnotationDataset, SocialSample, frame_path
...
def test_social_sample_canonicalizes_lah_direction_once():
    sample = SocialSample.from_manifest_record({
...
def test_every_manifest_annotation_becomes_one_sample(tmp_path):
    ...
    dataset = SocialAnnotationDataset(manifest)
...
def test_invalid_annotations_are_rejected(record, message):
    with pytest.raises(ValueError, match=message):
        SocialSample.from_manifest_record(record)
...
def test_duplicate_annotation_is_not_silently_oversampled(tmp_path):
    ...
    with pytest.raises(ValueError, match="duplicate pair annotation"):
        SocialAnnotationDataset(manifest)
