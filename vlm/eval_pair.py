...
Examples::
    ...
    python -m vlm.eval_pair run --mode raw_graph ...
    python -m vlm.eval_pair run --mode features_mlp --checkpoint .../best ...
    python -m vlm.eval_pair run --mode vlm --name vlm_lm_aux --checkpoint .../best ...
    python -m vlm.eval_pair compare result_dirs_or_json_files...
"""
...
def run_evaluation(args) -> dict[str, Any]:
    ...
    graph_metrics = evaluate_pair_predictions(
        args.gtmeta,
        graph_collector.probabilities,
        expected_sids=expected_sids,
        threshold=threshold,
    )
    print(format_pair_metrics(graph_metrics, "raw_graph"), flush=True)
    ...
    if mode == "raw_graph":
        ...
        criterion = PairTaskBCELoss().to(device)
        ...
        print(format_pair_metrics(metrics, args.name or mode), flush=True)
