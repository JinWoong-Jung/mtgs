...
class PairInputDataset(Dataset):
    ...
    def __init__(
        self,
        manifest: str | Path | PairAnnotationDataset,
        frame_root: str | Path,
    ...
    def _generative_rng(self, sample: PairSample):
...
def sample_graph_logit(sample: PairSample, cache: Mapping[str, object]) -> torch.Tensor:
...
def graph_confidence(sample: PairSample, cache: Mapping[str, object]) -> float:
...
class GraphControlDataset(Dataset):
    """Same labelled rows as PairInputDataset, with no image or Qwen dependency."""
...
class GraphFeatureControlDataset(Dataset):
    ...
    def __init__(
        ...
        # sample is independently validated by assemble_pair_graph_evidence.
        first = self.annotations.samples[0]
        self.feature_dim = assemble_pair_graph_evidence(
    ...
    def __getitem__(self, index: int) -> GraphFeatureControlInput:
        sample = self.annotations[index]
        evidence = assemble_pair_graph_evidence(
...
def pair_control_collate(items: list[GraphControlInput]) -> dict[str, object]:
    if not items:
