...
class PairSample:
    ...
    @classmethod
    def from_manifest_record(cls, record: Mapping[str, Any]) -> "PairSample":
...
class PairAnnotationDataset(Dataset):
    """Load every labelled manifest row as exactly one :class:`PairSample`.
        ...
        with self.manifest.open("r", encoding="utf-8") as stream:
            for line_number, line in enumerate(stream, start=1):
                ...
                try:
                    ...
                    sample = PairSample.from_manifest_record(record)
...
def frame_path(frame_root: str | Path, sample: PairSample) -> Path:
