...
CLI:
  python -m vlm.cache.render overlays --split <s> --out <dir> \
...
from vlm.cache.overlay import denormalize_to_pil, display_labels
...
def main():
    ...
    ap.add_argument(
        ...
        help="frozen vlm.cache.selection JSON; required for deterministic N-capped train/val extraction",
    )
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2 or sys.argv[1] != "overlays":
        sys.exit("usage: python -m vlm.cache.render overlays --split <s> --out <dir> --manifest <jsonl> --gtmeta <pt>")
    sys.argv.pop(1); main()
