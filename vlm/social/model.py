...
from vlm.social.evidence import GraphBatch, stack_graph_evidence
from vlm.social.input import SocialVLMInput
from vlm.runtime.vision_cache import VisionDiskCache
from vlm.social.projection import EvidenceProjector
...
def prepare_social_tokens(tokenizer: Any, model: nn.Module) -> dict[str, int]:
...
def _social_chat_text(processor: Any, item: SocialVLMInput) -> str:
...
def make_social_collate(processor: Any, *, reuse_vision: bool = False):
    ...
    def collate(items: Sequence[SocialVLMInput]) -> dict[str, Any]:
        ...
        texts = [_social_chat_text(processor, item) for item in items]
        if reuse_vision:
        ...
        out = dict(encoded)
        out["pair_graph"] = stack_graph_evidence([item.evidence for item in items])
...
def evidence_placeholder_masks(input_ids: torch.Tensor, token_ids: Mapping[str, int]):
    ...
    ids = torch.tensor(
        [token_ids[t] for t in SOCIAL_EVIDENCE_TOKENS],
...
class GenerativeVLM(nn.Module):
    ...
    def __init__(
        ...
        self._injector = _EvidenceInjectionHook(_find_language_model(backbone))
...
def placeholder_masks(
    ...
    ids = torch.tensor(
        [token_ids[token] for token in SOCIAL_SPECIAL_TOKENS],
...
class SocialVLM(_VisionReuseMixin, nn.Module):
    ...
    def __init__(
        ...
        self._injector = _EvidenceInjectionHook(language_model)
