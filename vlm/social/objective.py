...
from vlm.social.data import SOCIAL_TASK_ID, SOCIAL_TASKS
from vlm.social.evidence import GraphBatch
from vlm.social.model import GenerativeVLM, SocialVLM, SocialVLMOutput, TextGenerativeVLM
...
class TaskResidualDecoder(nn.Module):
...
class YesNoResidualHead(nn.Module):
...
class GraphLogitMLPControl(nn.Module):
    ...
    def __init__(self, hidden_dim: int = 32, dropout: float = 0.0):
        super().__init__()
        self.decoder = TaskResidualDecoder(
...
class GraphFeatureMLPControl(nn.Module):
    ...
    def _flat_features(self, graph: GraphBatch) -> torch.Tensor:
    ...
    def forward(
        self, graph: GraphBatch, task_ids: torch.Tensor
    ) -> DecoderOutput:
        batch = len(graph.tasks)
        _validate_task_ids(task_ids, batch)
...
class TaskBCELoss(nn.Module):
    ...
    def forward(
        ...
        return BCELossOutput(loss=per_sample.mean(), per_sample=per_sample)


@dataclass
class NextTokenLossOutput:
    loss: torch.Tensor
    per_sample: torch.Tensor
...
class SocialObjective(nn.Module):
    ...
    def forward(
        ...
        if labels is None:
            return SocialObjectiveOutput(vlm=vlm_output, decoder=decoder_output)
...
class GenerativeObjective(nn.Module):
    ...
    * eval: one prompt forward, then ``sigmoid(logit_yes - logit_no)`` at the
      answer position. Legacy graph-token runs retain teacher-forced candidates.
    """

    def __init__(
        self,
        vlm: GenerativeVLM | TextGenerativeVLM,
        *,
    ...
    def score(self, model_inputs: Mapping[str, torch.Tensor], num_pairs: int) -> torch.Tensor:
        """Return P(yes) via direct text logits or legacy teacher-forced candidates."""
        if self.direct_yes_no_token_ids is not None:
