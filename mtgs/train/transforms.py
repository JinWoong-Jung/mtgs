...
def get_img_size(image):
    ...
    if isinstance(image, Image.Image):
        ...
        raise Exception(
            f"Input image needs to be either a Image.Image or torch.Tensor. Found {type(image)} instead"
        )
    return img_w, img_h


class RandomHeadBboxJitter(object):
    """Applies random geometric transformations (ie. translations
    and/or expansions) to the head bounding box"""
...
class Resize(object):
    ...
    def __init__(self, img_size, head_size):
        ...
        self.head_size = (
            head_size
            if isinstance(head_size, (tuple, list))
    ...
    def __call__(self, sample):
        if torch.rand(1) <= self.p:
            ...
            if num_heads > 0:
                ...
                sample["gaze_pts"][sample["gaze_pts"][:, 0] != -1.0, 0] = (
                    1.0 - sample["gaze_pts"][sample["gaze_pts"][:, 0] != -1.0, 0]
                )
                valid_indices = torch.where(sample["head_bboxes"].sum(-1) != 0)[0]
                tmp = sample["head_bboxes"][valid_indices, 0].clone()
                sample["head_bboxes"][valid_indices, 0] = (
                    1.0 - sample["head_bboxes"][valid_indices, 2]
                )
                sample["head_bboxes"][valid_indices, 2] = 1.0 - tmp
    ...
    def __call__(self, sample):
        if torch.rand(1) <= self.p:
            ...
            brightness_factor = (
                ...
                else torch.rand(1).uniform_(*self.brightness)
            )
            contrast_factor = (
                None
                if self.contrast is None
                else torch.rand(1).uniform_(*self.contrast)
            )
            saturation_factor = (
                None
                if self.saturation is None
                else torch.rand(1).uniform_(*self.saturation)
            )
            ...
            if np.array(sample["image"]).sum() > 0:
                ...
                for fn_id in fn_indices:
                    if fn_id == 0 and brightness_factor is not None:
                        sample["image"] = TF.adjust_brightness(
                            sample["image"], brightness_factor
                        )
                        for k in range(num_heads):
                            if sample["head_bboxes"][k].sum() > 0:
                                sample["heads"][k] = TF.adjust_brightness(
                                    sample["heads"][k], brightness_factor
                                )

                    elif fn_id == 1 and contrast_factor is not None:
                        sample["image"] = TF.adjust_contrast(
                            sample["image"], contrast_factor
                        )
                        for k in range(num_heads):
                            if sample["head_bboxes"][k].sum() > 0:
                                sample["heads"][k] = TF.adjust_contrast(
                                    sample["heads"][k], contrast_factor
                                )

                    elif fn_id == 2 and saturation_factor is not None:
                        sample["image"] = TF.adjust_saturation(
                            sample["image"], saturation_factor
                        )
                        for k in range(num_heads):
                            if sample["head_bboxes"][k].sum() > 0:
                                sample["heads"][k] = TF.adjust_saturation(
                                    sample["heads"][k], saturation_factor
                                )
                        ...
                        for k in range(num_heads):
                            if sample["head_bboxes"][k].sum() > 0:
                                sample["heads"][k] = TF.adjust_hue(
                                    sample["heads"][k], hue_factor
                                )
...
class Normalize(object):
    def __init__(
        self, img_mean=[0.44232, 0.40506, 0.36457], img_std=[0.28674, 0.27776, 0.27995]
    ):
...
class RandomCropSafeGaze(object):
    """Randomly crops the input image while ensuring the gaze target
    and the head bounding box remain within the crop. The crop is also
    chosen such that it respects the given aspect ratio (or the aspect
    ratio of the image if None)"""
    ...
    def __call__(self, sample):
        if torch.rand(1) <= self.p:
            ...
            crop_xmin, crop_ymin, crop_w, crop_h = self._get_random_crop_bbox(
                img_w, img_h, sample["head_bboxes"], sample["gaze_pts"]
            )

            # Crop Image
            sample["image"] = TF.crop(
                sample["image"],
                int(crop_ymin),
                int(crop_xmin),
                int(crop_h),
            ...
            if num_heads > 0:
                # Convert Head Bboxes
                sample["head_bboxes"] = sample["head_bboxes"] * torch.tensor(
                    [[img_w, img_h]]
                ).repeat(1, 2)
                sample["head_bboxes"] = sample["head_bboxes"] - torch.tensor(
                    [[crop_xmin, crop_ymin]]
                ...
                # Convert Gaze Points
                mask = sample["gaze_pts"][:, 0] != -1.0
                sample["gaze_pts"][mask] = sample["gaze_pts"][mask] * torch.tensor(
                    [[img_w, img_h]]
                ) - torch.tensor([[crop_xmin, crop_ymin]])
                sample["gaze_pts"][mask] = sample["gaze_pts"][mask] / torch.tensor(
                    [[crop_w, crop_h]]
                )
    ...
    def _get_random_crop_bbox(self, img_w, img_h, head_bboxes, gaze_pts):
        """Computes the parameters of a random crop that maintains the
        aspect ratio, and includes the gaze point and head bounding box"""
        ...
        if len(gaze_pts) > 0:
            coords = torch.concat(
                [head_bboxes, gaze_pts[gaze_pts[:, 0] != -1.0].repeat(1, 2)], dim=0
            )
        if len(coords) > 0:
        ...
        zone_xmin, zone_ymin, zone_xmax, zone_ymax = self._expand(
        ...
        if xmin <= xmax:
            ...
            logger.info(f"(zone_h, zone_w) = ({zone_h}, {zone_w})")
            logger.info(f"(crop_h, crop_w) = ({crop_h}, {crop_w})")
