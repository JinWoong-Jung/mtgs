...
class Distance(tm.Metric):
    ...
    def __init__(self):
        ...
        self.add_state(
            "n_observations", default=torch.tensor(0.0), dist_reduce_fx="sum"
        )
    ...
    def update(
        ...
        if mask.any():
            self.sum_dist += (
                (gaze_point_gt[mask] - gaze_point_pred[mask]).pow(2).sum(1).sqrt().sum()
            )
...
class GFTestDistance(tm.Metric):
    ...
    def __init__(self):
        super().__init__()
        self.add_state(
            "sum_dist_to_avg", default=torch.tensor(0.0), dist_reduce_fx="sum"
        )
