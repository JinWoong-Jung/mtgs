...
class GraphGaze(nn.Module):
    def __init__(self, in_channels=768, hidden_channels=96, heads=8):
        ...
        self.conv1 = geom_nn.GATv2Conv(
            in_channels, hidden_channels, heads=heads, concat=True
        )
    ...
    def forward(self, x, num_valid_people):
        ...
        edge_index_list = self.build_edge_index(
            batch_size, num_people, num_valid_people, x.device
        )
            ...
            torch.tensor(
                list(
                    itertools.permutations(
                        torch.arange(
                            num_people - num_valid_people[i], num_people, device=device
                        ),
                        2,
                    )
                ),
    ...
    def build_graph_batch(self, x, edge_index_list):
        data_list = [
            geom_data.Data(x=x_i, edge_index=ei)
            for (x_i, ei) in zip(x, edge_index_list)
        ]
