...
class VideoCoAttDataset_temporal(Dataset):
    def __init__(
        ...
        self.annotations = pd.read_hdf(
            os.path.join(ann_root, f"videocoatt_{self.split}.h5"), "data"
        )
                ...
                for pi, pid in enumerate(person_ids):
                    ...
                    if len(pid_idx) == 0:
                        ...
                        try:
                            ...
                            logger.info("Error inout")
                            logger.info("inout:", img_annotations["inout"])
...
class VideoCoAttDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage: str):
        if stage == "fit":
            ...
            val_transform = Compose(
                [
                    Resize(img_size=224, head_size=self.head_size),
                    ToTensor(),
                    Normalize(img_mean=IMG_MEAN, img_std=IMG_STD),
                ]
            )
